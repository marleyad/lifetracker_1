server <- function(input, output) {
  ...
  observeEvent(input$submit_strength, {
    ...
    if (nrow(current_data) > 0 && 
      ...
      updated_strg_data <- rbind(current_data, new_entry_pushup)
    }
    ...
    tryCatch({
      dbWriteTable(con, "pushups", updated_strg_data, overwrite = TRUE)
      showNotification("Data successfully updated in the database", type = "message")
    }, error = function(e) {
